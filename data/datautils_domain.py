...
from data.confi_datasets import *
...
def build_dataset(set_id, transform,confi_imag,confi_dis, data_root, domain_name,mode='test', n_shot=None, split="all", bongard_anno=False):
    if set_id == 'I':
    ...
    elif set_id in domain_datasets:
        if mode == 'train' and n_shot:
            testset = build_domain_dataset(set_id,data_root, transform,domain_name, mode=mode, n_shot=n_shot)
        else:
            testset = build_domain_dataset(set_id,data_root, transform,domain_name, mode=mode)
    elif set_id =='sfuda':
        testset = build_confi_dataset(confi_imag, confi_dis,transform, mode=mode) # (图像, 真实标签, 伪标签, 索引)

    elif set_id in ['cifar100_c','cifar10_c']:
        testset = build_cifar_dataset(confi_imag, confi_dis,transform)
    elif set_id == 'bongard':
