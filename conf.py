...
_C.UAHLR.CLS_PAR = 0.4
_C.UAHLR.ENT = True
_C.UAHLR.GENT = True
_C.UAHLR.EPSILON = 1e-5
_C.UAHLR.GENT_PAR = 1.0
_C.UAHLR.CTX_INIT = 'a_photo_of_a' #initialize context 
_C.UAHLR.N_CTX = 4 
_C.UAHLR.ARCH = 'ViT-B/32' #['RN50', 'ViT-B/32','RN101','ViT-B/16']
_C.UAHLR.TTA_STEPS = 1
_C.UAHLR.IIC_PAR = 1.0
_C.UAHLR.LOAD = None
_C.UAHLR.CTR_PAR = 1.0 # 新加
_C.UAHLR.NUM_NEIGHBORS = 4 # 新加
_C.UAHLR.THRESHOLD = 0.8 # 新加 
_C.UAHLR.N = 4 # 新加 
_C.UAHLR.CUR_RATIO = 0.3 # 新加
...
def load_cfg_from_args():
    ...
    parser.add_argument('--SAVE_DIR', type=str, help='自定义输出文件夹') # 新加
    ...
    cfg.output_dir_src = os.path.join(cfg.CKPT_DIR,cfg.SETTING.OUTPUT_SRC,cfg.DA,cfg.SETTING.DATASET,cfg.domain[cfg.SETTING.S][0].upper())
     # 如果用户没有提供 SAVE_DIR，则使用默认生成的路径 新加
    if args.SAVE_DIR:
        cfg.output_dir = os.path.join(args.SAVE_DIR,cfg.DA,cfg.SETTING.DATASET,cfg.domain[cfg.SETTING.S][0].upper()+cfg.domain[cfg.SETTING.T][0].upper(),cfg.MODEL.METHOD)
    else:
    ...
    # cfg.output_dir = os.path.join(cfg.SAVE_DIR,cfg.DA,cfg.SETTING.DATASET,cfg.domain[cfg.SETTING.S][0].upper()+cfg.domain[cfg.SETTING.T][0].upper(),cfg.MODEL.METHOD)
    cfg.output_dir = os.path.join(cfg.output_dir,f"run_{current_time}") # 新加 
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(os.path.join(cfg.output_dir, "log.txt")), # 新加
