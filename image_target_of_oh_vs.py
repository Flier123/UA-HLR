...
import src.methods.oh.CausalDA as CausalDA
import src.methods.oh.difo as DIFO
import src.methods.oh.uahlr as UAHLR
...
if __name__ == "__main__":
    ...
    cfg.type = cfg.domain # ["amazon","dslr","webcam"]
    cfg.t_dset_path = cfg.FOLDER + cfg.SETTING.DATASET + '/' + cfg.domain[cfg.SETTING.T] + '_list.txt' # 目标域数据集路径
    cfg.test_dset_path = cfg.FOLDER + cfg.SETTING.DATASET + '/' + cfg.domain[cfg.SETTING.T] + '_list.txt'
    cfg.s_dset_path = cfg.FOLDER + cfg.SETTING.DATASET + '/' + cfg.domain[cfg.SETTING.S] + '_list.txt' # 源域数据集路径
    cfg.savename = cfg.MODEL.METHOD # difo
    start =time.time()
    ...
    elif cfg.MODEL.METHOD == "uahlr":
        print("using UAHLR method")
        if 'RN' in cfg.UAHLR.ARCH:
            acc = DIFO_50.train_target(cfg)
        else:
            acc = UAHLR.train_target(cfg)
