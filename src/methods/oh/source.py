...
def cal_acc_oda(cfg,loader, netF, netB, netC):
    ...
    return np.mean(acc[:-1]), np.mean(acc), unknown_acc # 已知类准确率，整体准确率，未知类准确率
    # return np.mean(acc), np.mean(acc[:-1])

def train_source(cfg):
    cfg.output_dir_src = f'./output/source_oda/uda/office-home/{cfg.domain[cfg.SETTING.S][0]}'
...
def test_target(cfg):
    ...
    for i in range(len(cfg.domain)):
        ...
        dset_loaders = data_load(cfg)
        if cfg.DA == 'oda':
            acc_os1, acc_os2, acc_unknown = cal_acc_oda(cfg, dset_loaders['test'], netF, netB, netC)
