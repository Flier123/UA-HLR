...
from src.data.data_list import ImageList_idx_adacon, NCropsTransform
from PIL import ImageFilter
...
def data_load(cfg): 
    ...
    if cfg.SETTING.DATASET=="domainnet126":
        dsets["test"] = ImageList_idx_adacon(txt_test, transform=image_test_domainnet126())
    else:
        dsets["test"] = ImageList_idx_adacon(txt_test, transform=image_test())
...
def train_target(cfg):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0:
            epoch = iter_num // len(dset_loaders["target"]) # 当前epoch
